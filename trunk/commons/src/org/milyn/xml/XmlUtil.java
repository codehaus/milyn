...
public class XmlUtil {
    ...
    /**
     * Remove all entities from the supplied <code>Reader</code> stream replacing
    ...
    /**
     * Remove all entities from the supplied <code>String</code> stream replacing
    ...
    /**
     * Rewrite all entities from the supplied <code>Reader</code> stream replacing
     * them with their character reference equivalents.
     * <p/>
     * Example: <b>&ampnbsp;</b> is rewriten as <b>&amp#160;</b>
     * <p/>
     * Both the read and write streams are returned unclosed.
     * @param reader The read stream.
     * @param writer The write stream.
    ...
    public static Document parseStream(InputStream stream, EntityResolver entityResolver, boolean validate, boolean expandEntityRefs) throws SAXException, IOException {
		...
		try {
			...
			IllegalStateException state = new IllegalStateException("Unable to parse XML stream - XML Parser not configured correctly.");
			state.initCause(e);
			throw state;
		} catch (FactoryConfigurationError e) {
			IllegalStateException state = new IllegalStateException("Unable to parse XML stream - DocumentBuilderFactory not configured correctly.");
			state.initCause(e);
			throw state;
		}
    }
     ...
     * @param node The document node to be searched.
     * @param xpath The XPath String to be used in the selection.
     * @return The W3C Node instance at the specified location in the
     * document, or null.
     */
	...
	public static String serialize(NodeList nodeList) throws DOMException {
		...
		try {
			...
			StringWriter writer = new StringWriter();
			StreamResult xmlStreamResult = new StreamResult(writer); 
			...
			for(int i = 0; i < listLength; i++) {
				...
				if(XmlUtil.isTextNode(node)) {
					...
					serializer.transform(new DOMSource(node), xmlStreamResult);
				}
			}			
