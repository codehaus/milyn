 ...
 * <li><code>destinationDirectoryPattern</code>: is the destination directory for files created by this router.   Supports templating.
 * <li><code>highWaterMark</code>: max number of output files in the destination directory at any time.
 * <li><code>highWaterMarkTimeout</code>: number of ms to wait for the system to process files in the destination
 * 		directory so that the number of files drops below the highWaterMark.
 * <li><code>highWaterMarkPollFrequency</code>: number of ms to wait between checks on the High Water Mark, while
