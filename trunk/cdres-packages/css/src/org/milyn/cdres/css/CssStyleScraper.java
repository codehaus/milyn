...
public class CssStyleScraper extends AbstractAssemblyUnit {
	...
	private void visitStyle(Element element, ContainerRequest request, String media) {
		...
		if(children != null && children.getLength() > 0) {
			...
			try {
				char[] cssData = new char[buffer.length()];
				CharArrayReader reader;
				
				buffer.getChars(0, buffer.length(), cssData, 0);
				reader = new CharArrayReader(cssData);
	...
	private void parseCSS(Element element, ContainerRequest request, String media, InputSource inputSource) throws DOMException, MalformedURLException {
		StyleSheet styleSheet;
		StyleSheetStore store;
		
		store = StyleSheetStore.getStore(request);
		styleSheet = store.getCssParser().parseStyleSheet(inputSource, request.getRequestURI().toURL(), (media != null?media:"mediaXXX"));
