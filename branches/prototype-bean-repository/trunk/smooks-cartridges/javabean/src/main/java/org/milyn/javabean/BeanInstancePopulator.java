...
import org.milyn.javabean.repository.RepositoryBeanIdList;
...
public class BeanInstancePopulator implements DOMElementVisitor, SAXElementVisitor {
    ...
    private RepositoryBeanId repositoryBeanId;
    
    private RepositoryBeanId wireBeanRepositoryBeanId;
    ...
    public void initialize() throws SmooksConfigurationException {
        ...
        RepositoryBeanIdList repositoryBeanIdList = beanRepositoryManager.getRepositoryBeanIdList();
        
        repositoryBeanId = repositoryBeanIdList.getRepositoryBeanId(beanId);
    ...
    private RepositoryBeanId getWireBeanRepositoryBeanId() {
    	if(wireBeanRepositoryBeanId == null) {
    		wireBeanRepositoryBeanId = beanRepositoryManager.getRepositoryBeanIdList().getRepositoryBeanId(wireBeanId);
    	}
    	return wireBeanRepositoryBeanId;
    }

    private void bindBeanValue(final ExecutionContext executionContext) {
    	final RepositoryBeanId targetRepositoryBeanId = getWireBeanRepositoryBeanId();
    	
    	final BeanRepository beanRepository = beanRepositoryManager.getBeanRepository(executionContext);
    	
    	Object bean = beanRepository.getBean(targetRepositoryBeanId);
        if(bean == null) {
        	...
        	beanRepository.addBeanLifecycleObserver(targetRepositoryBeanId, BeanLifecycle.BEGIN, getId(), false, new RepositoryBeanLifecycleObserver(){
                ...
                public void onBeanLifecycleEvent(RepositoryBeanLifecycleEvent event) {
                    ...
                    beanRepository.associateLifecycles(repositoryBeanId , targetRepositoryBeanId);
                    ...
                    if(wiredBeanType == Classification.ARRAY_COLLECTION ) {
                    	...
                    	beanRepository.addBeanLifecycleObserver( targetRepositoryBeanId, BeanLifecycle.CHANGE, getId(), true, new RepositoryBeanLifecycleObserver() {
                            public void onBeanLifecycleEvent(RepositoryBeanLifecycleEvent event) {
