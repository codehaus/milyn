 ...
 * <p/>
 * The profile configuration is supplied in a well defined XML format.  This XML format is defined in
 * <a href="doc-files/device-profile-1.0.txt">device-profile-1.0.dtd</a> and a sample
 * XML is defined in <a href="doc-files/device-profile-sample-1.0.txt">device-profile-sample.xml</a>.
 * <p/>
	...
	public ProfileStore parse(InputStream input) throws SAXException, IOException {
		...
		while(!XmlUtil.getString(profileDoc, profileSelector).equals("")) {
			...
			while(tokenizer.hasMoreTokens()) {
				...
				try {
					profileSet = (DefaultProfileSet)store.getProfileSet(device);
				} catch(UnknownDeviceException unknown) {
					profileSet = new DefaultProfileSet();
					try {
						store.addProfileSet(device, profileSet);
					} catch(IllegalArgumentException invalid) {
						SAXException saxE = new SAXException("<device-profile> attribute 'list' contains invalid device-name token value [" + device + "].");						
