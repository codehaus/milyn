 ...
 * XmlUtilTest
 * <p> Relations:
 *     XmlUtil extends java.lang.Object <br>
 *
 * @author Tom Fennelly
...
public class XmlUtilTest extends TestCase {
    ...
    /**
     * Test method: void removeEntities(Reader, Writer)
     * removeEntities throws java.io.IOException
     */
    ...
    public void testRemoveEntities_26() {
        try {
            assertEquals("a<a & & &g", XmlUtil.removeEntities("a&#X3C;a & &amp; &g"));
        } catch (Exception excep) {
            fail("Unhandled exception: " + excep.getMessage());
        }
    }
    ...
    public void testRewriteEntities_01() {
        try {
            StringReader reader = new StringReader("a&yen;a  &#171; hh & ; jgha& &amp; gaa &j");
            StringWriter writer = new StringWriter();

            XmlUtil.rewriteEntities(reader, writer);
            assertEquals("a&#165;a  &#171; hh & ; jgha& &#38; gaa &j", writer.toString());
        } catch (Exception excep) {
            fail("Unhandled exception: " + excep.getMessage());
        }
    }
    ...
    /**
     * This method is required because the tests can be run from either ANT
    ...
    private File getDTDfolder() {
    	...
    	try {
	    	...
	    	String parentFolder = currentFolder.getAbsoluteFile().getParentFile().getName(); 
	    	
	    	if(parentFolder.equalsIgnoreCase("milyn")) {
	    		// We're running the test from within eclipse IDE.
	    		return (new File("./commons/test/dtd"));
	    	} else if(parentFolder.equalsIgnoreCase("ant")) {
	    		// We're running the test from ANT
				return (new File("../test/dtd"));
	    	} else {
	    		fail("Can't run test - can't identify working directory.  Working directory: " + currentFolder.getAbsolutePath());
	    	}
    	} catch(Exception e) {
    		e.printStackTrace();
    	}    	
...
// UnitTest using its constructors - these are copies of the constructors in XmlUtil.
// In sumation, construct and use XmlUtil.UnitTest in your unit test in the same way as you'd expect to use
// XmlUtil.
//
/*
public static class UnitTest {
   XmlUtil testTargetInst = null;

   public UnitTest()  {
     testTargetInst = new XmlUtil();
   }

   public UnitTest(XmlUtil testTarget) {
     setTestTarget(testTarget);
   }

   public void setTestTarget(XmlUtil testTarget) {
     testTargetInst = testTarget;
   }

   //----------------------------------------------------------------------

   public static void removeEntities (java.io.Reader param0,java.io.Writer param1)  throws java.io.IOException {
     XmlUtil.removeEntities( param0, param1);
   }
}
*/
