...
/**
 * Test that the getResource method tries to load based on the rules in the following 
 * order: 
 * 1. First tries loading the resource based on the servlet config (init-param)
 * parameter if present.
 * 2. Checks for a Servlet Context parameter (context-param) and tries to load 
 * based on this if present.
 * 3. Tries based on the default supplied in the call.
 * @author tfennelly
 */
public class ServletResourceLocatorTest extends TestCase {
	...
	public void testGetResource_exceptions() {
		...
		ServletResourceLocator servletLocator = new ServletResourceLocator(config, null);  
		...
		try{
			...
			try {
				...
				assertEquals("Resource [paramName] not specified in configuration, plus no default load location provided.", e.getMessage());
			}
	...
	/**
	 * Test that getResource loads based on the servlet config param - <init-param>
	 */
	public void testGetResource_initparam() {
		MockServletConfig config = getServletConfig();
		MyMockServletContext context = ((MyMockServletContext)config.getServletContext());
	...
	/**
	 * Test that getResource loads based on the servlet context param - <context-param>
	 */
	...
	/**
	 * Test that getResource loads from the external resource locator
	 * by making the resource stream load from the context fail.
	 */
	public void testGetResource_fromURLResourceLocator() {
		MockServletConfig config = getServletConfig();
		MyMockServletContext context = ((MyMockServletContext)config.getServletContext());
		MockExternalResourceLocator resLocator = new MockExternalResourceLocator();
		ServletResourceLocator servletLocator = new ServletResourceLocator(config, resLocator);		
