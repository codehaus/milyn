...
public class ServletParamUtilsTest extends TestCase {
	...
	public void testGetResource_initparam() {
		...
		((MockServletContext)config.getServletContext()).setInitParameter("paraX", "paraX-context");		
		try {
			assertEquals("paraX-config", ServletParamUtils.getParameterValue("paraX", config));
		} catch (Exception e) {
			fail(e.getMessage());
		}
	}
	...
	public void testGetResource_contextparam() {
		...
		((MockServletContext)config.getServletContext()).setInitParameter("paraX", "paraX-context");		
		try {
			assertEquals("paraX-context", ServletParamUtils.getParameterValue("paraX", config));
		} catch (Exception e) {
			fail(e.getMessage());
		}
	}
