...
/**
 * {@link java.net.URI} resource locator. <p/> Loads resources from a
 * {@link java.net.URI} i.e. "file://", "http://", "classpath:/" etc. <p/> Note,
 ...
 * a "/org/milyn/x/my-resource.xml" resource on the classpath. "classpath" is in
 * fact the default scheme for this class i.e. if no scheme is specified in the
 * supplied URI, this class defaults to treating the URI as a reference to a
 * classpath based resource. <p/> All resource URIs are
 * {@link URI#resolve(String) resolved} against a "base URI" which can be set
 * through the System by the "org.milyn.resource.baseuri" System property.
 * 
 * @author tfennelly
 */
public class URIResourceLocator implements ContainerResourceLocator {
	...
	private InputStream getResource(URI uri) throws IllegalArgumentException,
			IOException {
		URL url;
		...
		if (scheme == null || scheme.equals(SCHEME_CLASSPATH)) {
			...
			if (path == null) {
				throw new IllegalArgumentException(
						"Unable to locate classpath resource [" + uri
			...
			if (stream == null) {
				throw new IOException(
						"Failed to access data stream for classpath resource ["
								+ path + "].");
			}
		} else {
			url = uri.toURL();
			...
			if (stream == null) {
				throw new IOException("Failed to access data stream for "
						+ uri.getScheme() + " resource [" + uri + "].");
			}
		}
	...
	/**
	 * Resolve the supplied uri against the suppled baseURI. <p/> Only resolved
	 * against the base URI if 'uri' is not absolute.
	 * 
	 * @param uri
	 *            URI to be resolved.
	 * @param baseURI
