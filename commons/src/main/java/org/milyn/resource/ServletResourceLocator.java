...
public class ServletResourceLocator implements ContainerResourceLocator {
	 ...
	 * @param externalResourceLocator Resource locator for resources outside the 
	 * Servlet context.
	 */
	 ...
	 * <p/>
	 * If a Servlet configuration (see {@link ServletParamUtils#getParameterValue(String, ServletConfig)}) exists 
	 * for "configName" the method uses use the config value, otherwise it use the "defaultUri" value provided.  
	 * Calls {@link #getResource(String)} to get the resource stream.
	 */
	public InputStream getResource(String configName, String defaultUri) throws IllegalArgumentException, IOException {
        ...
        if(configVal == null) {
        	...
        	if(defaultUri != null) {
                config.getServletContext().log("[Milyn] Defaulting resource [" + configName + "] load location to: " + defaultUri);
                configVal = defaultUri;
        	} else {
                throw new IllegalArgumentException("Resource [" + configName + "] not specified in configuration, plus no default load location provided.");
        	}
        } else {
            config.getServletContext().log("[Milyn] Resource [" + configName + "] load location[servlet-context]: " + configVal);
        }
        ...
        if(resourceStream == null) {
            if(configVal == defaultUri) {
                ...
                throw new IllegalArgumentException("Invalid resource parameter [" + configName + "=" + configVal + "] defined in deployment descriptor.  Unable to access specified resource.");
            }
        }
	...
	public InputStream getResource(String uri) throws IllegalArgumentException, IOException {
		InputStream resourceStream = config.getServletContext().getResourceAsStream(uri);

		if(resourceStream == null) {
        	// Might be a config resource in which case it might be in the WEB-INF folder.
        	resourceStream = config.getServletContext().getResourceAsStream("/WEB-INF" + uri);
        }
