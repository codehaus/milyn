...
public class LocalEntityResolver implements EntityResolver {
	...
	/**
	 * DTD entity lookup table.
	 * <p/>
	 * Contains preread DTD entity byte arrays. 
	 */
	 ...
	 * Public default Constructor
	 * <p/>
	 * This constructor allows specification of a local file system folder 
	 * from which DTDs can be loaded.
	...
	public LocalEntityResolver(File localDTDFolder) {
		...
		if(!localDTDFolder.exists()) {
			throw new IllegalStateException("Cannot resolve local DTD entities.  Local DTD folder not present: [" + localDTDFolder.getAbsolutePath() + "].");
		}
	...
	public InputSource resolveEntity(String publicId, String systemId) throws SAXException, IOException {
		byte cachedBytes[] = (byte[])LocalEntityResolver.dtdEntities.get(systemId);
		...
		if(cachedBytes == null) {
			...
			// First try locate the file in the DTD folder based on the files full path.
			// If this fails, try locate it in the root of the DTD folder directly. If
			// this too fails try the classpath - specifically the org.milyn.dtd package.
			if(localDTDFolder != null) {  
	 ...
	 * @param stream Entity stream to be read.
