...
public class HeaderAction extends AbstractContentDeliveryUnit {
	...
	public HeaderAction(CDRDef cdrDef) {
		...
		String actionParam = cdrDef.getStringParameter("action");
		if(actionParam == null) {
			throw new IllegalStateException("'action' param not defined on HeaderAction CDRDef.");
		}
		...
		if(actionParam.equals("add")) {
			...
			throw new IllegalStateException("Unsupported header action '" + actionParam + "' defined on HeaderAction CDRDef.");
		}
		
		headerName = cdrDef.getStringParameter("header-name");
		if(headerName == null) {
			throw new IllegalStateException("'header-name' param not defined on HeaderAction CDRDef.");
		}
		
		headerValue = cdrDef.getStringParameter("header-value");
		if(action == ACTION_ADD && headerValue == null) {
			throw new IllegalStateException("'header-value' param not defined on 'add' HeaderAction CDRDef.");
		}
	}	
