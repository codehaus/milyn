...
public class SmooksEDIReader extends EDIParser implements SmooksXMLReader {
	 ...
	 * Logger.
	 */
	private static Log logger = LogFactory.getLog(SmooksEDIReader.class);
	/**
	 * Context lookup key for the mapping model table.
	 */
	private static String MAPPING_TABLE_CTX_KEY = SmooksEDIReader.class.getName() + "#MAPPING_TABLE_CTX_KEY";
	/**
	...
	private EdifactModel getMappingModel() throws IOException, SAXException {
		EdifactModel edi2xmlMappingModel;
		Hashtable mappings = getMappingTable(executionContext.getContext());

		synchronized (configuration) {
			edi2xmlMappingModel = (EdifactModel) mappings.get(configuration);
			if(edi2xmlMappingModel == null) {
				...
				logger.info("Parsed, validated and cached EDI mapping model [" + edi2xmlMappingModel.getEdimap().getDescription().getName() + ", Version " + edi2xmlMappingModel.getEdimap().getDescription().getVersion() + "].  Target Profile(s) " + getTargetProfiles() + ".");
			} else if(logger.isInfoEnabled()) {
				logger.info("Found EDI mapping model [" + edi2xmlMappingModel.getEdimap().getDescription().getName() + ", Version " + edi2xmlMappingModel.getEdimap().getDescription().getVersion() + "] in the model cache.  Target Profile(s) " + getTargetProfiles() + ".");
			}
		}
