...
class UNHHandler implements ControlBlockHandler {
    ...
    public void process(InterchangeContext interchangeContext) throws IOException, SAXException {
		...
		MappingRegistry reg = interchangeContext.getMappingRegistry();

		interchangeContext.getControlSegmentParser().startElement(InterchangeContext.INTERCHANGE_MESSAGE_BLOCK_ELEMENT_NAME, NAMESPACE, true);
		...
		EdifactModel mappingModel = UNEdifactUtil.getMappingModel(messageName, segmentReader.getDelimiters(), reg);
		...
		interchangeContext.getControlSegmentParser().endElement(InterchangeContext.INTERCHANGE_MESSAGE_BLOCK_ELEMENT_NAME, NAMESPACE, true);
	}
